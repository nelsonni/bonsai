(function() {
  ...
  QUnit.test("Single Card tests", function(assert) {
    ...
    assert.ok(cardHeader[0].classList.contains("card-header"), "Header found on canvas");
    assert.ok(cardHeader[0].classList.contains("card"), "Card header found to be apart of card");
    ...
    assert.equal(curCard[0].style.left, "110px", "Card succesfully dragged horizontally");
    ...
    assert.equal(document.getElementsByClassName("actualCard").length, 0, "Card sucessfully deleted from canvas");
    $(curCard).remove();
  });
  ...
  QUnit.test("Stacked Cards tests", function(assert) {
    ...
    assert.equal(document.getElementsByClassName("highlightBox").length, 0, "HighlightBox removed Successfully from canvas");
    assert.equal(document.getElementsByClassName("actualCard").length, 2, "2 cards found on the canvas.");
    $("#closeBtn2").simulate("click");
    assert.equal(document.getElementsByClassName("actualCard").length, 1, "1 cards found on the canvas.");
    $("#closeBtn1").simulate("click");
    assert.equal(document.getElementsByClassName("actualCard").length, 0, "No cards found on the canvas.");
  });
  ...
  function handleWrappedCards(wrapped, assert) {
    ...
    $(wrapped).each(function(idx) {
      if (this.classList.contains("actualCard") && !this.classList.contains("lastFit")) {
        assert.equal(this.style.top, (parseInt(lastFit.style.top) - (20 * (idx))).toString() + "px", "Overflow cards stacked back positioned top correctly");
        assert.equal(this.style.left, (parseInt(lastFit.style.left) - (5 * (idx))).toString() + "px",
          "Overflow cards stacked back positioned left correctly");
      }
    });
  }
  ...
  function expansionOK(box, assert) { // need to account for card wrap around still
    ...
    for (var i = 0; i < box[0].children.length; i++) {
      if (box[0].children[i].classList.contains("actualCard")) {
        ...
        assert.equal(curCard.style.top, (parseInt(box[0].style.top) + 10).toString() + "px", "Card positioned top correctly on expansion");
        assert.equal(curCard.style.left, (parseInt(box[0].style.left) + (225 * (i) + 5)).toString() + "px",
          "Card position left correctly on expansion");
      }
    }
  }
